    ...
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 flex flex-col gap-6">
          <motion.div className="liquid-glass rounded-3xl p-6 border border-foreground/10">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-foreground/10">
              <div className="p-2.5 rounded-xl bg-indigo-500/10 text-indigo-500"><ShieldCheck className="w-5 h-5" /></div>
              <div>
                <h2 className="text-lg font-bold text-foreground">Authentication Policy</h2>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors ${mfaEnabled ? 'bg-indigo-500' : 'bg-foreground/20'}`}
                >
                  <motion.div 
                    layout 
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 text-xs font-bold rounded-lg border border-emerald-500/20">Enforced</span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button className="px-4 py-2 bg-foreground/5 hover:bg-foreground/10 text-foreground text-xs font-bold rounded-xl transition-colors border border-foreground/10">Configure</button>
              </div>
            </div>
          </motion.div>

          <motion.div className="liquid-glass rounded-3xl p-6 border border-foreground/10 flex-1 flex flex-col">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-foreground/10">
              <div className="p-2.5 rounded-xl bg-foreground/5 text-foreground"><History className="w-5 h-5" /></div>
              <div>
            ...
            <div className="flex-1 overflow-y-auto space-y-4">
              {AUDIT_LOG.map((log) => (
                <div key={log.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl bg-foreground/5 border border-foreground/5 hover:border-foreground/10 transition-colors gap-4">
                  ...
                  <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 sm:gap-1 w-full sm:w-auto text-xs">
                    <div className="flex items-center gap-1.5 text-muted font-mono bg-background/50 px-2 py-0.5 rounded-md">
        ...
        <div className="flex flex-col gap-6">
          <motion.div whileHover={organicInteractions.hover} className="liquid-glass rounded-3xl p-6 border border-rose-500/30 bg-rose-500/5 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><AlertTriangle className="w-24 h-24 text-rose-500" /></div>
            <div className="relative z-10">
              <h2 className="text-lg font-bold text-rose-500 mb-2">High Risk Alert</h2>
              ...
              <button className="w-full py-2.5 bg-rose-500 hover:bg-rose-600 text-white font-bold text-sm rounded-xl transition-colors shadow-lg shadow-rose-500/20">
          ...
          <motion.div className="liquid-glass rounded-3xl p-6 border border-foreground/10 flex-1">
            <h2 className="text-sm font-bold text-muted uppercase tracking-wider mb-6">Active Sessions</h2>
            <div className="space-y-4">
              <div className="p-4 rounded-xl border border-indigo-500/30 bg-indigo-500/5">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-500 text-[10px] font-bold rounded uppercase">Active</span>
                </div>
                <p className="text-xs text-muted mb-1">Mac OS • Chrome 124.0</p>
                <p className="text-xs font-mono text-muted">192.168.1.105 (Mumbai)</p>
              </div>
              ...
              <div className="p-4 rounded-xl border border-foreground/10 bg-foreground/5">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <button className="text-[10px] font-bold text-rose-500 hover:text-rose-600 uppercase">Revoke</button>
                </div>
                <p className="text-xs text-muted mb-1">iOS 17 • Mobile App</p>
                <p className="text-xs font-mono text-muted">Last sync: 2 hours ago</p>
              </div>
            </div>
            <button className="w-full mt-6 py-2.5 bg-foreground/5 hover:bg-rose-500/10 text-rose-500 font-bold text-sm rounded-xl transition-colors border border-foreground/10 hover:border-rose-500/30">
