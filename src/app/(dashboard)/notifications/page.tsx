...
import { Bell, ShieldAlert, CheckCircle2, Info, Clock, MailOpen, Trash2, Check, ArrowUpRight } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="flex flex-col md:flex-row gap-6 flex-1 min-h-0">
        ...
        <div className="w-full md:w-96 liquid-glass rounded-3xl border border-foreground/10 flex flex-col h-full overflow-hidden shrink-0">
          <div className="p-4 border-b border-foreground/10 bg-foreground/5 flex items-center justify-between">
            <h2 className="font-bold text-foreground">Inbox</h2>
            <button className="text-xs font-semibold text-indigo-500 hover:text-indigo-600">Mark all as read</button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {NOTIFICATIONS.map(note => (
              <button
                ...
                className={`w-full text-left p-3 rounded-xl transition-all duration-300 flex gap-3 ${
                  activeNote === note.id ? 'bg-foreground/10 specular-edge' : 'hover:bg-foreground/5'
                }`}
              >
                <div className={`mt-0.5 p-2 rounded-xl shrink-0 ${
                  note.type === 'critical' ? 'bg-rose-500/10' :
                  note.type === 'success' ? 'bg-emerald-500/10' :
                  note.type === 'warning' ? 'bg-amber-500/10' : 'bg-indigo-500/10'
                }`}>
                  {getIcon(note.type)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-0.5">
                    ...
                    {!note.read && <div className="w-2 h-2 rounded-full bg-indigo-500 shrink-0 mt-1 ml-2"></div>}
                  </div>
                  <p className="text-xs text-muted truncate">{note.desc}</p>
                  <p className="text-[10px] text-muted font-bold uppercase tracking-wider mt-1.5">{note.time}</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 liquid-glass rounded-3xl border border-foreground/10 flex flex-col overflow-hidden relative">
          <AnimatePresence mode="wait">
            {activeData ? (
              ...
              >
                <div className="p-6 border-b border-foreground/10 bg-foreground/5 flex justify-between items-start">
                  <div>
                    <h1 className="text-2xl font-bold text-foreground mb-2">{activeData.title}</h1>
                  ...
                  <div className="flex gap-2">
                    <button className="p-2 rounded-lg bg-foreground/5 border border-foreground/10 text-muted hover:text-foreground transition-colors"><MailOpen className="w-4 h-4" /></button>
                    <button className="p-2 rounded-lg bg-foreground/5 border border-foreground/10 text-rose-500 hover:bg-rose-500/10 transition-colors"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
                  ...
                  <div className="max-w-2xl text-foreground/80 space-y-4">
                    ...
                    {activeData.type === 'critical' && (
                      <div className="mt-8 p-4 rounded-2xl bg-rose-500/5 border border-rose-500/20 text-sm">
                        <p className="font-mono text-rose-500 mb-2">Error Log Snippet:</p>
                        <pre className="p-3 bg-black/80 text-rose-300 rounded-xl overflow-x-auto text-xs">
                          {`[ERROR] 2026-05-09T10:15:22Z
POST /webhook/invoice
Status: 500 Internal Server Error
Response: "Database connection timed out during execution"`}
                        </pre>
                        <button className="mt-4 px-4 py-2 bg-rose-500 text-white rounded-lg text-xs font-bold shadow-lg shadow-rose-500/20 flex items-center gap-2">
